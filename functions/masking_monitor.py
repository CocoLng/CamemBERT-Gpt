...
class MaskingHandler:
    def __init__(self, data_loader):
        ...
        self.mlm_probability = 0.15  # Probabilité de masquage par défaut
    ...
    def load_streaming_dataset(self, size_gb: float) -> str:
        ...
        try:
            ...
            if not self.data_loader.dataset:
                raise ValueError(f"Échec du chargement du dataset {self.data_loader.dataset_config.name}")

            # Configuration du pipeline de streaming avec extraction des données
            self.data_loader.dataset = self.data_loader.dataset.shuffle(buffer_size=optimal_buffer)
            self.data_loader.dataset = self.data_loader.dataset.map(
                extract_text,
                remove_columns=[col for col in self.data_loader.dataset.column_names if col != 'text']
            )
            ...
            # Log personnalisé
            self.logger.info(f"Dataset {self.data_loader.dataset_config.name} chargé avec succès avec une taille de {size_gb} GB")
        ...
        try:
            ...
            for _ in range(max_samples):
                try:
                    ...
                    sample_tensor = {k: torch.tensor(v, dtype=torch.long) for k, v in sample.items() if isinstance(v, (list, np.ndarray))}
                    ...
                    self.logger.warning(f"Erreur lors du traitement de l'échantillon: {e}")
                    continue

            if samples_checked == 0:
                raise ValueError("Aucun échantillon valide trouvé pour la vérification du masquage")
            ...
            self.logger.error(f"Erreur dans la vérification du masquage: {e}", exc_info=True)
            raise
    ...
    def visualize_with_density(self, text: str, density: float) -> Tuple[str, str]:
        ...
        try:
            if text.strip():
                ...
                sample = next(iter(self.data_loader.dataset))
                text = self.data_loader.tokenizer.decode(sample['input_ids'], skip_special_tokens=True)
    ...
    def _format_loading_status(self, size_gb: float, stats: MaskingStats, buffer_size: int) -> str:
        ...
        return (f"✅ Dataset streaming initialisé avec succès !\n"
